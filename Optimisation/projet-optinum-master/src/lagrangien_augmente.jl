...
function lagrangien_augmente(f::Function, gradf::Function, hessf::Function, 
    ...
    while Non_arret
        La(x) = f(x) + λ' * c(x) + (μ / 2) * norm(c(x)) ^ 2
        gradLa(x) = gradf(x) + λ' * gradc(x) + μ * c(x) * gradc(x)
        hessLa(x) = hessf(x) + λ' * hessc(x) + μ * (gradc(x) * gradc(x)' +  c(x) * hessc(x))
        if (algo_noc == "newton")
            x_sol,fs,fl,iters,xs = newton(La, gradLa, hessLa, x_sol)
        elseif (algo_noc == "rc-gct")
            x_sol,fs,fl,iters,xs = regions_de_confiance(La, gradLa, hessLa, x_sol, algo_pas = "gct")
        elseif (algo_noc == "rc-cauchy")
            x_sol,fs,fl,iters,xs = regions_de_confiance(La, gradLa, hessLa, x_sol, algo_pas = "cauchy")
        end
        if (norm(c(x_sol)) <= η)
            λ = λ + μ * c(x_sol)
            λs = vcat(λs, λ)
        ...
        nb_iters = nb_iters + 1
        gradLNak = gradf(x_sol) + λ' * gradc(x_sol)
        gradLNa0 = gradf(x0) + λ0' * gradc(x0)
        critere1 = norm(gradLNak) <= max(tol_rel*norm(gradLNa0), tol_abs)
        critere2 = norm(c(x_sol)) <= max(tol_rel*norm(c(x0)), tol_abs)
        critere3 = norm(gradLa(x_sol)) <= ε
