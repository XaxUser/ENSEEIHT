...
public class MaxTabPool {
    ...
    private static int computeMax(ExecutorService executor, int[] array, int numberOfTasks) throws InterruptedException, ExecutionException {
        ...
        for (int i = 0; i < numberOfTasks; i++) {
            ...
            int end = (i == numberOfTasks - 1) ? array.length : Math.min((i + 1) * taskSize, array.length);
            PartialMax task = new PartialMax(array, start, end);
        ...
        for (Future<Integer> futureResult : results) {
            int partialMax = futureResult.get(); // Attend et récupère le résultat de la tâche
            max = Math.max(max, partialMax); // Met à jour le maximum global
        }
        return max;
    }
